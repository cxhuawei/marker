...
DEBUG_OPTS = [cfg.BoolOpt(
    "marker-debug",
    default=False,
    help="Print debugging output only for marker. "
...
log.addLevelName(log.RDEBUG, "MARKERDEBUG")
...
def setup(product_name, version="unknown"):
    ...
    if CONF.marker_debug:
        oslogging.getLogger(
            project=product_name).logger.setLevel(log.RDEBUG)


class MarkerContextAdapter(oslogging.KeywordArgumentAdapter):
...
def log_deprecated(message, marker_version, log_function=None, once=False):
    """A wrapper marking a certain method as deprecated.

    :param message: Message that describes why the method was deprecated
    :param marker_version: version of Marker when the method was deprecated
    :param log_function: Logging method to be used, e.g. LOG.info
...
def log_deprecated_args(message, marker_version, deprecated_args,
    ...
    :param marker_version: version of Marker when the arguments were deprecated
...
def is_debug():
    return CONF.debug or CONF.marker_debug
